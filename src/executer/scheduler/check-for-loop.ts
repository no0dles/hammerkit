...
import { hasDependencyCycle, hasMixedCycle, hasNeedCycle } from '../../planner/validate'
import { WorkTree } from '../../planner/work-tree'

export function checkForLoop(workTree: WorkTree): void {
  for (const task of iterateWorkTasks(workTree)) {
    const cyclePath = hasDependencyCycle(task, []) ?? hasMixedCycle(task, [])
  ...
  for (const service of iterateWorkServices(workTree)) {
    const cyclePath = hasNeedCycle(service, []) ?? hasMixedCycle(service, [])
    if (cyclePath && cyclePath.length > 0) {
