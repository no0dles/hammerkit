...
export async function runTaskDocker(image: string, task: TaskNode, arg: RunArg) {
  ...
  const addVolume = (volume: ContainerMount) => {
    if (
      !volumes.some(
        (v) => v.localPath === volume.localPath || v.containerPath === volume.containerPath,
      )
    ) {
      volumes.push(volume);
    }
...
async function execCommand(arg: RunArg, docker: Dockerode, container: Container, cwd: string, cmd: string[], cmdName: string, user: string | undefined) {
