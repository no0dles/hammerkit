...
import {
  WorkNodeAbortedState, WorkNodeCancelState,
  WorkNodeCompletedState, WorkNodePendingState,
...
export function runNode(workTree: WorkTree, nodeId: string, context: ExecutionContext) {
...
export function completeNode(workTree: WorkTree, nodeId: string, context: ExecutionContext) {
...
export function failNode(workTree: WorkTree, nodeId: string, context: ExecutionContext, error: Error) {
  const node = workTree.nodes[nodeId];
  ...
  if (currentState.type === 'running') {
    const newState: WorkNodeState = canceledExecution ?
      {type: 'aborted'} : {type: 'failed', ended: new Date(), duration: getDuration(node.status.state), error};
...
export function resetNode(workTree: WorkTree, nodeId: string, context: ExecutionContext) {
  const node = workTree.nodes[nodeId];
...
export function cancelNodes(workTree: WorkTree, context: ExecutionContext) {
