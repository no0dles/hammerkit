...
function makeContainer(remove: Mock): Container {
...
describe('removeContainer', () => {
  ...
  it('swallows 404 (already gone)', async () => {
    ...
    const remove = vi.fn().mockRejectedValueOnce(err)
  ...
  it('retries on 409 (conflict)', async () => {
    ...
    const remove = vi.fn().mockRejectedValueOnce(conflict).mockResolvedValueOnce(undefined)
    await removeContainer(makeContainer(remove))
